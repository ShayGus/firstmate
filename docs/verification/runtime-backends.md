...
On 2026-09-24, the guard was extended to omp and run in an isolated worktree with `FM_COMPOSER_MATRIX_LIVE=1 tests/fm-composer-matrix-live-e2e.test.sh`; it exited with failures. Installed omp 18.3.0 returned `unknown` instead of `empty`: its live pane showed a bordered `⬢ GPT-6-Sol · ◑ med` composer, a different shape from the bare `❯` omp 18.2.11 footer captured for this change. Codex 0.156.1 passed anchored and cursorless reads; OpenCode 1.18.32 passed anchored and returned cursorless `unknown`. Claude 2.1.280, Pi 0.86.1, and Muse 1.3.0 stopped at workspace trust prompts in this worktree. Grok, Kimi, and Zellij were absent. This run does not live-verify the 18.2.11 matcher or establish that current omp reaches `empty`; rerun the guard against a trusted workspace and investigate the 18.3.0 composer shape separately.
