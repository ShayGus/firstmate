...
E=/home/shayg/.no-mistakes/evidence/01M39SN6G9D40EFA3V9GBC85A0
...
launch() {  # <label> <extra omp args...>
  local label=$1; shift
  local tab pane
  tab=$(lab tab create --workspace w2 --cwd /tmp/fm-omp-lab-proj --label "$label" --no-focus) || return 1
  pane=$(printf '%s' "$tab" | jq -r '.result.root_pane.pane_id')
  lab pane run "$pane" "env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u GEMINI_CLI -u CURSOR_AGENT -u CURSOR_INVOKED_AS FM_OMP_HARNESS=omp OMP_SKIP_SETUP=1 omp --config $R/.omp/fm-worker-overlay.yml $* --auto-approve --cwd /tmp/fm-omp-lab-proj" >/dev/null
  printf '%s\n' "$pane"
}
"$@"
